...
function SourcesPage() {
  ...
  useEffect(() => { if (!API_BASE_URL) return; void fetch(`${API_BASE_URL}/api/sources`).then((response) => response.ok ? response.json() : null).then((data) => { if (Array.isArray(data)) setSources(data as SourceSummary[]); }).catch(() => undefined); }, []);
