"""
Akasa Air scraper — Playwright + network interception.
"""
...
class AkasaScraper(BaseFlightScraper):
    ...
    async def search(self, params: FlightSearchParams) -> list[FlightResult]:
        ...
        url = (
            f"https://www.akasaair.com/booking/flight-search"
            f"?from={params.origin}&to={params.destination}"
            f"&date={dep}&adult={params.passengers}&child=0&infant=0&tripType=OW"
        )
...
def _parse(data: Any, params: FlightSearchParams) -> list[FlightResult]:
    ...
    for item in raw:
        ...
        results.append(FlightResult(
            id=str(uuid.uuid4()),
            source="Akasa Air", airline="Akasa Air",
...
def _find_list(data):
    ...
    for path in (
        ...
        ("results",), ("journeys",), ("itineraries",), ("data",),
    ):
        node = data
        for k in path:
...
def _fare(item):
    for k in ("totalFare", "totalAmount", "totalPrice", "fare", "price", "total"):
        v = item.get(k)
        if v is not None:
            try:
                return float(v)
