"""
SpiceJet scraper — Playwright + network interception.
"""
...
class SpiceJetScraper(BaseFlightScraper):
    ...
    async def search(self, params: FlightSearchParams) -> list[FlightResult]:
        ...
        url = (
            f"https://www.spicejet.com/?from={params.origin}&to={params.destination}"
            f"&depart={dep}&adult={params.passengers}&child=0&infant=0&tripType=OW"
        )
        ...
        async with async_playwright() as p:
            ...
            async def handle(r: Response) -> None:
                if any(k in r.url for k in ("search", "availability", "flight", "FlightSearch")):
                    try:
...
def _parse(data: Any, params: FlightSearchParams) -> list[FlightResult]:
    ...
    for item in raw:
        ...
        results.append(FlightResult(
            id=str(uuid.uuid4()),
            source="SpiceJet", airline="SpiceJet",
            flight_number=_str(item, "flightNumber", "FlightNumber", "flightNo"),
...
def _find_list(data):
    ...
    for path in (
        ...
        ("flights",), ("results",), ("itineraries",), ("data",),
    ):
        node = data
        for k in path:
...
def _fare(item):
    for k in ("TotalFare", "totalFare", "totalAmount", "totalPrice", "fare", "Fare", "price"):
        v = item.get(k)
        if v is not None:
            try:
                return float(v)
...
def _times(item, params):
    dep = item.get("departureTime") or item.get("DepTime") or item.get("depTime") or item.get("departure")
    arr = item.get("arrivalTime") or item.get("ArrTime") or item.get("arrTime") or item.get("arrival")
