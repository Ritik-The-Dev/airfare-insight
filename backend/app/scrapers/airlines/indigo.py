"""
IndiGo scraper — Playwright + network interception.

Navigates to IndiGo's search page and intercepts the internal availability
API response. Raises ScraperUnavailableError on any failure.
"""
...
class IndiGoScraper(BaseFlightScraper):
    ...
    async def search(self, params: FlightSearchParams) -> list[FlightResult]:
        ...
        url = (
            ...
            f"&departDate={dep}&adult={params.passengers}&child=0&infant=0"
            f"&tripType=O&intl=false&cabinClass=ECONOMY"
        )
        ...
        async with async_playwright() as p:
            ...
            async def handle_response(response: Response) -> None:
                if any(k in response.url for k in ("availability", "search", "flight")):
                    try:
...
def _parse(data: Any, params: FlightSearchParams) -> list[FlightResult]:
    ...
    for item in raw:
        ...
        results.append(FlightResult(
            id=str(uuid.uuid4()),
            source="IndiGo", airline="IndiGo",
            flight_number=_str(item, "flightNumber", "flightNo", "fn"),
            origin=params.origin, destination=params.destination,
            departure_time=dep_dt, arrival_time=arr_dt,
            duration_minutes=duration_minutes(dep_dt, arr_dt),
            stops=_int(item, "stops", "stopCount", "layovers"),
...
def _fare(item: dict) -> float | None:
    for k in ("totalFare", "totalAmount", "totalPrice", "fare", "price", "amount", "total"):
        v = item.get(k)
        if v is not None:
            try:
                return float(v)
                ...
                pass
    # nested fare.totalFare
    for nested in ("fare", "price", "Fare"):
        sub = item.get(nested)
        if isinstance(sub, dict):
            for k2 in ("totalFare", "total", "TF", "TotalFare"):
                v = sub.get(k2)
                if v is not None:
                    try:
                        return float(v)
...
def _times(item: dict, params: FlightSearchParams):
    dep = item.get("departureTime") or item.get("depTime") or item.get("departure") or item.get("dep")
    arr = item.get("arrivalTime") or item.get("arrTime") or item.get("arrival") or item.get("arr")
