"""
ixigo scraper — Playwright + network interception.
"""
...
class IxigoScraper(BaseFlightScraper):
    source_name = "ixigo"
    source_type = "OTA"
    ...
    async def search(self, params: FlightSearchParams) -> list[FlightResult]:
        ...
        url = (
            ...
            f"/{params.origin}/{params.destination}/{dep}"
            f"/null/{params.passengers}/0/0/E/0/CHEAPEST/DOM"
        )
        ...
        async with async_playwright() as p:
            ...
            async def handle(r: Response) -> None:
                if any(k in r.url for k in ("flight", "search", "result", "api/v")):
                    try:
...
def _parse(data: Any, params: FlightSearchParams) -> list[FlightResult]:
    ...
    for item in raw:
        ...
        if not dep_dt or not arr_dt:
            continue
        airline = str(item.get("airline") or item.get("airlineName") or item.get("carrier") or "Unknown")
        results.append(FlightResult(
            id=str(uuid.uuid4()),
            source="ixigo", airline=airline,
            flight_number=_str(item, "flightNumber", "flight_number", "flightNo"),
...
def _find_list(data):
    ...
    if not isinstance(data, dict):
        return None
    for path in (
        ("data", "flightResults"), ("data", "flights"),
        ("flightResults",), ("flights",), ("results",),
        ("journeys",), ("itineraries",), ("data",),
    ):
        node = data
        for k in path:
...
def _fare(item):
    for k in ("totalFare", "totalAmount", "totalPrice", "fare", "price", "amount"):
        v = item.get(k)
        if v is not None:
            try:
                return float(v)
