"""
Cleartrip scraper — Playwright + network interception.
"""
...
class CleartripScraper(BaseFlightScraper):
    ...
    async def search(self, params: FlightSearchParams) -> list[FlightResult]:
        ...
        url = (
            ...
            f"&depart_date={dep}&adults={params.passengers}"
            f"&childs=0&infants=0&class=Economy&sd=1"
        )
        ...
        async with async_playwright() as p:
            ...
            async def handle(r: Response) -> None:
                if any(k in r.url for k in ("search", "results", "flight", "listing")):
                    try:
...
def _parse(data: Any, params: FlightSearchParams) -> list[FlightResult]:
    ...
    for item in raw:
        ...
        if not dep_dt or not arr_dt:
            continue
        airline = str(item.get("airline") or item.get("carrier") or item.get("airlineName") or "Unknown")
        results.append(FlightResult(
            id=str(uuid.uuid4()),
            source="Cleartrip", airline=airline,
            flight_number=_str(item, "flightNumber", "flight_number"),
